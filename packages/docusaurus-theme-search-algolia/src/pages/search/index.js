...
function Search() {
  ...
  const {versioningEnabled, versions} = useVersioning();
  const [searchVersion, setSearchVersion] = useState(versions[0]);
  ...
  algoliaHelper.on(
    ...
    ({results: {query, hits, page, nbHits, nbPages}}) => {
      ...
      searchResultStateDispatcher({
        type: 'update',
        value: {
          items: page === 0 ? items : searchResultState.items.concat(items),
          totalResults: nbHits,
          totalPages: nbPages,
          lastPage: page,
  ...
  const handleSearchInputChange = (e) => {
    ...
    if (e.target.tagName === 'SELECT') {
      ...
      updateSearchParam(searchInputValue);
    }
  };
    ...
    <Layout title={getTitle()}>
      <div className="container margin-vert--lg">
        ...
        {searchResultState.items.length > 0 ? (
          ...
          searchQuery !== '' && <p>No results were found</p>
        )}
